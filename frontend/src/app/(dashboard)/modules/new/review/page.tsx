...
export default function ModuleReviewPage() {
  ...
  useEffect(() => {
    if (
      ...
      // Fetch source details to get titles for auto-generated name
      const generateContent = async () => {
        let moduleName = name;

        // If no name set, try to get it from the first source's title
        if (!moduleName && pendingSourceIds.length > 0) {
          try {
            const firstSource = await sourcesApi.get(pendingSourceIds[0]);
            if (firstSource.title) {
              // Use title without file extension if it's a filename
              moduleName = firstSource.title.replace(/\.[^/.]+$/, "");
            }
          } catch (error) {
            console.error("Error fetching source for name:", error);
          }
        }

        // Fall back to default if still no name
        moduleName = moduleName || "New Module";

        previewContent.mutate(
          { source_ids: pendingSourceIds, name: moduleName },
          {
            onSuccess: (data) => {
              setGeneratedContent(
                data.overview,
              ...
              // Update name with the auto-generated one
              if (!name) {
                setModuleField("name", moduleName);
              }
            },
          }
        );
